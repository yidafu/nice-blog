...
import dev.yidafu.blog.common.dto.BasePage
...
class SyncTaskService(
  ...
  suspend fun getSyncLogs(query: PageQuery): Pair<Int, List<SyncTaskModel>> = runDB{
    val logCount = context.selectCount().from(B_SYNC_TASK).fetchOne(0, Int::class.java) ?: 0
    val taskRecords = context.selectFrom(B_SYNC_TASK)
      .limit(query.size)
