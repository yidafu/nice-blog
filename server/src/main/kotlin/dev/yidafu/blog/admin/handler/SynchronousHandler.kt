...
class SynchronousHandler(
  ...
  suspend fun startSync(ctx: RoutingContext) {
    ...
    val syncCtx = GitConfig("https://github.com/yidafu/example-blog.git", branch = "master")
      ...
      if (log != null) {
        if (log.status != SyncTaskStatus.Running) {
          ...
          if (currentLog != null) {
            ...
            println(
              "${previewLog.isEmpty()}   current log ${currentLog.length},  preview log ${previewLog.length},  appendText ${appendText.length}",
            )
