...
inline fun FlowContent.tabs(
  ...
  val layout: TABS.() -> Unit = {
    ul {
      style = kw.inline { flex.col; }
      options.forEach { option ->
        tabItem(option)
      }
    }
    div("shadow-lg") {
      style = kw.inline { margin.left[6]; padding.y[4].x[8]; flex.grow }
      block()
    }
  }
  return TABS(
...
fun UL.tabItem(title: String, url: String, icon:
Icon, active: Boolean) {
  li("nav-list") {
    a {
      style = kw.inline {
        flex.row.items_center; padding.x[4].y[3]; border.rounded[LG];
        if (active) {
          text.white;
      ...
      href = url
      i {
        style = kw.inline { width[4].height[4]; margin.right[2] }
        icon.render(this@i)
      }
      + title
    }
